...
class sub_convert:
    ...
    def convert(
        ...
        if input_type == 'url':  # 获取 URL 订阅链接内容
            ...
            if isinstance(raw_input, list):
                ...
                for url in raw_input:
                    ...
                    try:
                        ...
                        s_content = sub_convert.yaml_decode(
                            sub_convert.transfer(resp.content.decode('utf-8'))
                        )
                ...
                try:
                    ...
                    sub_content = sub_convert.transfer(
        ...
        if sub_content != '订阅内容解析错误':  # 输出
            ...
            final_content = sub_convert.makeup(
                sub_content, dup_rm_enabled, format_name_enabled, speedtest
            )
            if output_type == 'YAML':
    ...
    def transfer(sub_content):  # 将 URL 内容转换为 YAML 格式
        if '</b>' not in sub_content:
            if (
                'proxies:' in sub_content
            ):  # 判断字符串是否在文本中，是，判断为YAML。https://cloud.tencent.com/developer/article/1699719
                ...
                url_content = sub_convert.yaml_encode(
                    sub_convert.format(sub_content))
                return url_content
            else:  # 判断 Base64.
                try:
                    url_content = sub_convert.base64_decode(sub_content)
                    url_content = sub_convert.yaml_encode(
                        sub_convert.format(url_content)
                    )
                    return url_content
    ...
    def format(
        sub_content, output=False
    ):  # 对节点 Url 进行格式化处理, 输出节点的字典格式, output 为真时输出 YAML 文本
        ...
        if 'proxies:' not in sub_content:  # 对 URL 内容进行格式化处理
            ...
            try:
                ...
                for url in raw_url_list:
                    while (
                        len(re.split('ss://|ssr://|vmess://|trojan://|vless://', url))
                        > 2
                    ):
                        url_to_split = url[8:]
                        if (
                            'ss://' in url_to_split
                            and 'vmess://' not in url_to_split
                            and 'vless://' not in url_to_split
                        ):
            ...
            if output == False:
                for item in sub_content_yaml['proxies']:  # 对转换过程中出现的不标准配置格式转换
                    try:
                        if (
                            item['type'] == 'vmess'
                            and 'HOST' in item['ws-headers'].keys()
                        ):
                            item['ws-headers']['Host'] = item['ws-headers'].pop(
                                "HOST")
                    except KeyError:
    ...
    def makeup(
        input, dup_rm_enabled=True, format_name_enabled=True, speedtest=True
    ):  # 对节点进行区域的去重筛选和重命名以及测速，输出 YAML 文本
        ...
        if isinstance(input, dict):
            ...
            if 'proxies:' in input:
                ...
                yaml_content_raw = sub_convert.convert(
                    input, 'content', 'YAML')
        ...
        if speedtest:  # 测速
            ...
            for proxy in proxies_list:
                ...
                if (
                    ...
                    ping_result[1] < 1
                    or ping_result_g[1] < 1
                    or ping_result_y[1] < 1
                    or ping_result_w[1] < 1
                ):
                    proxies_list.remove(proxy)
        ...
        for proxy in proxies_list:  # 改名
            if format_name_enabled:
                ...
                if country_code in emoji_plus:
                    name_emoji_plus = emoji_plus[country_code]
                else:
                    name_emoji_plus = emoji_plus['NOWHERE']

                proxy_index = proxies_list.index(proxy)
                # if proxy['type'] == 'vmess':
                #    proxy_type = 'VM'
                #    proxy['name'] = f'[{proxy_type}_{country_code}][{name_emoji}_{ip}]'
                # elif proxy['type'] == 'ss':
                #    proxy_type = 'SS'
                #    proxy['name'] = f'[{proxy_type}_{country_code}][{name_emoji}_{ip}]'
                ...
                if proxy['type'] == 'vmess':
                    proxy_type = 'VM'
                    proxy['name'] = f'[{proxy_type}][{name_emoji}][{name_emoji_plus}]'
                elif proxy['type'] == 'ss':
                    proxy_type = 'SS'
                    proxy['name'] = f'[{proxy_type}][{name_emoji}][{name_emoji_plus}]'
                elif proxy['type'] == 'trojan':
        ...
        for line in lines:
            ...
            if 'vmess://' in line:
                try:
                    ...
                    if vmess_config['id'] == '' or vmess_config['id'] is None:
                        ...
                        if (
                            vmess_config['net'] == ''
                            or vmess_config['net'] is False
                            or vmess_config['net'] is None
                        ):
                        ...
                        if (
                            vmess_config['path'] == ''
                            or vmess_config['path'] is False
                            or vmess_config['path'] is None
                        ):
                        ...
                        if (
                            vmess_config['tls'] == ''
                            or vmess_config['tls'] is False
                            or vmess_config['tls'] is None
                        ):
            ...
            if 'ssr://' in line:
                try:
                    ...
                    obfsparam = sub_convert.base64_decode(
                        param_dic['obfsparam'])
                    ...
                    yaml_url.setdefault(
                        'obfs-param', sub_convert.base64_decode(
                            param_dic['obfsparam'])
                    )
                    yaml_url.setdefault(
                        'protocol-param',
...
#                if (
#                    proxy['type'] == 'vmess'
#                ):  # Vmess 节点提取, 由 Vmess 所有参数 dump JSON 后 base64 得来。
#
#                    yaml_default_config = {
#                        'name': 'Vmess Node',
#                        'server': '0.0.0.0',
...
#                    vmess_proxy = str(
#                        'vmess://'
#                        + sub_convert.base64_encode(vmess_raw_proxy)
#                        + '\n'
#                    )
...
#                            trojan_go = (
#                                f'?security=tls&type={network_type}&headerType=none'
#                            )
#                        elif proxy['tls'] == False and proxy['network'] != 'tcp':
#                            trojan_go = (
#                                f'??allowInsecure=0&type={network_type}&headerType=none'
#                            )
#                    else:
...
#
#               # elif proxy['type'] == 'ssr':  # SSR 节点提取, 由 ssr_proxy 中参数再加上 需要的其他东西
#               #     ssr_base64_decoded = (
#               #         proxy['server']
#               #         + ':'
#               #         + proxy['port']
...
#               #         + proxy['obfs']
#               #         + ':'
#               #         + str(sub_convert.base64_encode(proxy['password']))
#               #         + '/?obfsparam='
#               #         + str(sub_convert.base64_encode(proxy['obfs-param']))
#               #         + '&protoparam='
#               #         + str(sub_convert.base64_encode(proxy['protocol-param']))
#               #         + '&remarks='
#               #         + str(sub_convert.base64_encode((proxy['name'])))
#               #         + '&group='
#               #         + str(sub_convert.base64_encode(proxy['group']))
#               #     )
#               #     ssr_proxy = str(
#               #         'ssr://'
#               #         + str(sub_convert.base64_encode(ssr_base64_decoded))
#               #         + '\n'
#               #     )
#               #     protocol_url.append(ssr_proxy)
    ...
    def base64_decode(url_content):  # Base64 转换为 URL 链接内容
        ...
        # print(len(url_content))
        try:
            base64_content = base64.b64decode(url_content.encode('utf-8')).decode(
                'utf-8', 'ignore'
            )  # https://www.codenong.com/42339876/
    ...
    def convert_remote(
        url='', output_type='clash', host='https://api.v1.mk'
        ...
        if output_type == 'clash':
            converted_url = (
                ...
                + url
                + '&insert=false&config=https%3A%2F%2Fraw.nameless13.com%2Fapi%2Fpublic%2Fdl%2FzKF9vFbb%2Feasy.ini'
            )
            try:
            ...
            if resp.text == 'No nodes were found!':
                ...
                sub_content = sub_convert.makeup(
                    sub_convert.format(resp.text),
                    dup_rm_enabled=False,
